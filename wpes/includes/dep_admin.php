        ...
        if($level==1){
            $statment=mysql_query("SELECT *FROM university");
                while($row=mysql_fetch_array($statment)){
                    ...
                    echo'<a data-toggle="collapse" href="adddeptocoll.php?c='.$row2['id'].'&uni='.$row2['uni'].'" target="content" class="btn btn-info btn-block" data-target="#c'.$row2['id'].'">'.$row2['name'].'</a>';
                    $col2=$row2['id'];
                    ...
                    while($row3=mysql_fetch_array($statment3)){
                        echo'<a class="btn btn-success btn-block" href="depshow2.php?dep='.$row3['id'].'" target="content">'.$row3['name'].'</a>';  
                    }
